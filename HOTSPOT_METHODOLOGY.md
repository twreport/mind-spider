...
```
emerging（萌芽）→ rising（上升）→ confirmed（确认热点）
                                  ↘ faded（衰退消失）
```
...
个性化不应该侵入通用热点检测系统。在能力化架构中，**客户过滤**是五个核心能力之一，与其他能力平等并行，通过共享数据层获取热点数据，独立计算客户相关性：
...
| 信号检测 | 硬编码算法发现异动，维护早期预警库 | `BroadTopicExtraction/analyzer/` |
...
深层采集
  ├── 事件：热点确认（confirmed）→ 全平台爬取充实报告
  ├── 事件：早期预警（rising）→ 定向爬取验证是否真的在发酵
  ├── 客户：品牌关键词 → 定期搜索品牌提及
  ├── 定时：高优先级话题 → 持续追踪讨论变化
  └── 反馈：表层发现新话题 → 深层验证和充实
...
**反馈环 1：深层采集 → 信号检测（发现水面下的信号）**

深层采集在社交平台搜索某个话题，发现大量讨论但该话题从未出现在任何热搜榜上。这个「水面下」的讨论量本身就是一个信号，应该反馈给信号检测层，创建新的候选话题。

**反馈环 2：深层采集 → 候选话题验证（提升预警准确率）**

早期预警库中的候选话题，仅靠热榜数据难以判断是否真的会爆发。深层采集可以去社交平台搜索相关关键词，用实际讨论量和情绪数据来验证候选话题的爆发潜力。

**反馈环 3：话题分析 → 深层采集（LLM 指导爬取方向）**

LLM 分析后判断某个话题可能有二次爆发风险（比如等待官方回应），触发深层采集持续追踪该话题。采集结果反馈给下一次 LLM 分析，形成「分析→采集→再分析」的循环。

**反馈环 4：指纹库 → 信号检测（自适应阈值）**
...
| 信号检测 | 硬编码算法实时发现异动，维护早期预警库 | `BroadTopicExtraction/analyzer/` |
| 话题分析 | LLM 深度分析、语义聚类、趋势研判 | `BroadTopicExtraction/analyzer/` |
| 客户过滤 | 个性化相关性评分，支持品牌/地方/行业客户 | 待实现 |

这些能力不是串行步骤，而是可以被多种触发源（定时、事件、客户需求、反馈数据）独立激活的服务。它们通过共享数据层交换信息，通过四条反馈环形成闭环：深层数据反哺信号检测、深层数据验证早期预警、LLM 分析指导爬取方向、指纹库驱动自适应阈值。
